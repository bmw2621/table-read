...
describe("Character Service", () => {
  ...
  beforeEach(() => {
    ...
    (db.select as jest.Mock).mockImplementation(() => ({
      from: jest.fn().mockReturnValue({
        where: jest.fn().mockResolvedValue([mockCharacter]),
      }),
    }));
  ...
  describe("createCharacter", () => {
    it("should create a character when user has access to script", async () => {
      const character = await createCharacter(mockScriptId, mockUserId);

      expect(getScript).toHaveBeenCalledWith(mockScriptId);
      expect(canAccessScript).toHaveBeenCalledWith(mockUserId, mockScript);
      expect(db.insert).toHaveBeenCalledWith(characters);
    ...
    it("should throw error when script does not exist", async () => {
      ...
      await expect(createCharacter(mockScriptId, mockUserId)).rejects.toThrow(
        "Script not found"
      );
  ...
  describe("getCharactersByScript", () => {
    it("should return characters for a script when user has access", async () => {
      const result = await getCharactersByScript(mockScriptId, mockUserId);
    ...
    it("should throw error when script does not exist", async () => {
      ...
      await expect(getCharactersByScript(mockScriptId, mockUserId)).rejects.toThrow(
        "Script not found"
      );
